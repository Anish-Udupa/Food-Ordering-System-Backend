...
router.get("/populate", async (req, res) => {
    ...
    try {
        ...
        categories.forEach(category => {
            default_menu[category].forEach(async (item) => {
                const menu_obj = new MenuModel({
                    title: item.title,
                    desc: item.desc,
                    imgsrc: item.imgsrc,
                    price: item.price,
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        categories.forEach(category => {
            const category_items = menu.filter(item => item.category === category);
            result.menu[category] = category_items.map(item => ({
                    item_id: item._id,
                    title: item.title,
                    desc: item.desc,
                    imgsrc: item.imgsrc,
                    imgalt: `Image of a ${item.title}`,
                    price: item.price,
                })
            );
        });
